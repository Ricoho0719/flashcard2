document.addEventListener('DOMContentLoaded', () => {
  ...
  let originalCardIndices = []; // Maps display index -> original index
  ...
  function loadSavedFlashcards() {
    ...
    .then(data => {
      ...
      console.log(`Loaded ${Object.keys(savedFlashcards).length} saved flashcards from API`);
  ...
  function loadSavedFlashcardsFromLocalStorage() {
    try {
      ...
      console.log(`Loaded ${Object.keys(savedFlashcards).length} saved flashcards from localStorage`);
      updateSaveButtonState();
    } catch (error) {
  ...
  function loadFlashcards() {
    if (topic === 'random') {
      ...
      Promise.all(
        topicFiles.map(t => fetch(`data/${t}.json`)
          .then(res => {
            if (!res.ok) {
              console.warn(`Failed to load ${t}.json (${res.status})`);
              return { flashcards: [] };
            }
            return res.json();
          })
          .catch(error => {
            console.error(`Error loading ${t}.json:`, error);
            return { flashcards: [] };
          })
        )
      )
      .then(dataArr => {
        ...
        if (combined.length === 0) {
          showErrorMessage("No flashcards found for any topic. Please check your data files.");
          return;
        }
        
        console.log(`Combined ${combined.length} flashcards from all topics for random mode`);
      ...
      fetch(`data/${topic}.json`)
        .then(res => {
          if (!res.ok) {
            throw new Error(`Failed to load ${topic}.json (${res.status})`);
          }
          return res.json();
        })
        .then(data => {
          if (!data.flashcards || !Array.isArray(data.flashcards) || data.flashcards.length === 0) {
            throw new Error(`No flashcards found in ${topic}.json`);
          }
          initFlashcards(data.flashcards);
        })
        .catch(error => {
          console.error("Error loading flashcard data:", error);
          showErrorMessage(`Could not load flashcards for ${topic}. Please try another topic.`);
        });
    }
  }

  // Fisher-Yates shuffle algorithm that tracks original indices
  function shuffleArray(array) {
    // Initialize original indices if not already done
    if (originalCardIndices.length !== array.length) {
      originalCardIndices = Array.from({ length: array.length }, (_, i) => i);
    }
    
    // Shuffle both the cards and keep track of original indices
    for (let i = array.length - 1; i > 0; i--) {
      ...
      // Swap original indices to match
      [originalCardIndices[i], originalCardIndices[j]] = 
        [originalCardIndices[j], originalCardIndices[i]];
    }
    
    console.log("Shuffled array. Original indices map:", originalCardIndices);
    return array;
  }
  ...
  function initFlashcards(data) {
    ...
    originalCardIndices = Array.from({ length: flashcards.length }, (_, i) => i);
  ...
  function updateCardCompletionStatus() {
    // Get original card index to mark the correct card as completed
    const originalIndex = originalCardIndices[currentCardIndex];
    const cardId = `${topic}_${originalIndex}`;
  ...
  function updateSaveButtonState() {
    const updateBtn = (btn) => {
      ...
      // Get original card index (before shuffling)
      const originalIndex = originalCardIndices[currentCardIndex];
      console.log(`Current display index: ${currentCardIndex}, Original index: ${originalIndex}`);
      
      const cardKey = `${topic}_${originalIndex}`;
  ...
  function updateButtonStates() {
    ...
    // Flip button
    if (flipBtn) {
      flipBtn.textContent = isShowingAnswer ? 'Question' : 'Answer';
      const flipIcon = flipBtn.querySelector('.material-icons');
      if (flipIcon) {
        flipIcon.textContent = isShowingAnswer ? 'refresh' : 'visibility';
      }
    }
  }
  ...
  // Save flashcard button event handlers
  function setupSaveButton(btn) {
    btn?.addEventListener('click', (e) => {
      e.stopPropagation(); // Prevent card click event
      console.log('Save button clicked:', btn.id); // Debug
      
      // Get original card index (before shuffling)
      const originalIndex = originalCardIndices[currentCardIndex];
      console.log('Current display index:', currentCardIndex, 'Original index:', originalIndex);
      
      // Use original index for the card key
      const cardKey = `${topic}_${originalIndex}`;
      const isSaved = savedFlashcards[cardKey] !== undefined;
      
      // Toggle saved state in memory first for immediate feedback
      if (isSaved) {
        delete savedFlashcards[cardKey];
        showNotification("Removing flashcard from saved items...", "default");
      } else {
        savedFlashcards[cardKey] = Date.now(); // Use timestamp as temporary ID
      ...
      if (!token) {
        ...
        showNotification(isSaved ? "Flashcard removed from saved items" : "Flashcard saved successfully", 
      ...
      if (isSaved) {
        ...
        console.log('Attempting to unsave card:', cardKey, savedId); // Debug
        ...
        .then(res => {
          console.log('Unsave response:', res.status); // Debug
          if (!res.ok) {
        ...
        .then(data => {
          console.log('Unsave data:', data); // Debug
          showNotification("Flashcard removed from saved items", "default");
          saveFlashcardsToLocalStorage(); // Update local storage as backup
        })
        .catch(err => {
          console.error("Error removing saved flashcard:", err);
          showNotification("Changes saved locally", "info");
          saveFlashcardsToLocalStorage(); // Still save locally
        });
      } else {
        // Save the card with original index
        console.log('Attempting to save card:', cardKey); // Debug
        ...
        fetch('/api/save-flashcard', {
          ...
          body: JSON.stringify({
            topic,
            cardIndex: originalIndex // Use original index here
          })
        })
        .then(res => {
          console.log('Save response:', res.status); // Debug
          if (!res.ok) {
        ...
        .then(data => {
          console.log('Save data:', data); // Debug
